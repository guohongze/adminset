...
def create_asset_excel(export, asset_id_all):
    if export == "true":
        if asset_id_all:
            ...
            writer.writerow([str2gb('主机名'), str2gb('IP地址'), str2gb('其它IP'), str2gb('所在机房'),
                             str2gb('资产编号'), str2gb('设备类型'), str2gb('设备状态'), str2gb('操作系统'),
                             str2gb('设备厂商'), str2gb('CPU型号'), str2gb('CPU核数'), str2gb('内存大小'),
                             str2gb('硬盘信息'), str2gb('SN号码'), str2gb('所在位置'),
                             str2gb('备注信息')])
            for h in asset_find:
                if h.asset_type:
...
def node_status(request, ids):
    ...
    def get_status_with_timeout(host_name, result):
        try:
            cpu_data = GetSysData(host_name, "cpu", 1800)
    ...
    # 使用线程和超时机制
    host = Host.objects.get(id=ids)
    result['hostname'] = host.hostname  # 将主机名添加到结果中
    
    thread = threading.Thread(target=get_status_with_timeout, args=(host.hostname, result))
    thread.daemon = True  # 设置为守护线程，避免主程序退出时线程仍在运行
    thread.start()
